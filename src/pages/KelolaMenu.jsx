...
import { ArrowLeft, Plus, Trash2, ToggleLeft, ToggleRight, AlertCircle } from 'lucide-react'
...
export default function KelolaMenu() {
  ...
  async function handleTambah() {
    ...
    const duplikat = menus.find(m => m.name.toLowerCase() === newName.trim().toLowerCase())
    if (duplikat) return setError('Menu sudah ada di daftar.')

    setSaving(true)
    const { error } = await supabase.from('menus').insert({ name: newName.trim() })
    if (!error) { setNewName(''); fetchMenus() }
    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="px-4 -mt-4 flex flex-col gap-4 pb-8">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <p className="text-sm font-semibold text-gray-700 mb-3">Tambah Menu Baru</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100">
            <p className="text-sm font-semibold text-gray-700">
              Daftar Menu <span className="text-gray-400 font-normal">({menus.length} menu)</span>
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-gray-400 text-sm">Belum ada menu. Tambah dulu!</p>
            </div>
          ) : (
            <div className="divide-y divide-gray-50">
              {menus.map(menu => (
                <div key={menu.id} className="flex items-center gap-3 px-4 py-3.5">
                  {/* Nama */}
                  <p className={`flex-1 text-sm font-medium ${menu.is_active ? 'text-gray-800' : 'text-gray-400 line-through'}`}>
                    {menu.name}
                  </p>

                  {/* Badge aktif/nonaktif */}
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                    menu.is_active ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'
                  }`}>
                    {menu.is_active ? 'Aktif' : 'Nonaktif'}
                  </span>

                  {/* Toggle */}
                  <button onClick={() => toggleAktif(menu)} className="text-gray-400 hover:text-green-500 transition-colors">
                    {menu.is_active
                  ...
                  {/* Hapus */}
                  <button onClick={() => handleHapus(menu.id)} className="text-gray-300 hover:text-red-400 transition-colors">
                    <Trash2 size={16} />
                  </button>
                </div>
              ))}
