...
export default function InputTransaksi() {
  ...
  async function handleSubmit() {
    // Validasi
    if (!form.type) return setStatus({ type: 'error', msg: 'Pilih jenis transaksi dulu.' })
    if (!form.amount || Number(form.amount) <= 0) return setStatus({ type: 'error', msg: 'Nominal harus lebih dari 0.' })
    if (form.type === 'expense' && !form.category) return setStatus({ type: 'error', msg: 'Pilih kategori pengeluaran.' })
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="bg-green-600 px-4 pt-10 pb-6 md:hidden">
        ...
        <p className="text-green-100 text-sm mt-1">Pilih jenis dan isi nominal</p>
      </div>
      ...
      <div className="px-4 -mt-4 flex flex-col gap-4 pb-8">
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <p className="text-sm font-medium text-gray-700 mb-3">Jenis Transaksi</p>
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              <div className={`p-2 rounded-full ${form.type === 'income' ? 'bg-green-100' : 'bg-gray-200'}`}>
                <TrendingUp size={24} className={form.type === 'income' ? 'text-green-600' : 'text-gray-400'} />
              </div>
              <span className={`font-semibold text-sm ${form.type === 'income' ? 'text-green-600' : 'text-gray-500'}`}>
                Pemasukan
              </span>
            </button>
            ...
            >
              <div className={`p-2 rounded-full ${form.type === 'expense' ? 'bg-red-100' : 'bg-gray-200'}`}>
                <TrendingDown size={24} className={form.type === 'expense' ? 'text-red-500' : 'text-gray-400'} />
              </div>
              <span className={`font-semibold text-sm ${form.type === 'expense' ? 'text-red-500' : 'text-gray-500'}`}>
                Pengeluaran
              </span>
            </button>
          </div>
        ...
        {form.type === 'expense' && (
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
            <p className="text-sm font-medium text-gray-700 mb-3">Kategori Pengeluaran</p>
            <div className="grid grid-cols-2 gap-2">
              {KATEGORI.map(kat => (
                <button
                  key={kat}
                  onClick={() => setForm(prev => ({ ...prev, category: kat }))}
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col gap-4">
            ...
            <div>
              ...
              <textarea
                ...
                value={form.note}
                onChange={handleChange}
                placeholder={form.type === 'income' ? 'Contoh: pendapatan penjualan siang' : 'Contoh: beli ayam dan tempe'}
                rows={3}
        ...
        {form.type && (
          ...
          >
            {loading ? 'Menyimpan...' : `Simpan ${form.type === 'income' ? 'Pemasukan' : 'Pengeluaran'}`}
          </button>
        )}
