...
import { TrendingUp, TrendingDown, Wallet, RefreshCw, FileDown } from 'lucide-react'
import {
      ...
      <div className="px-4 -mt-4 flex flex-col gap-4 pb-6">
        ...
        <div className="grid grid-cols-1 gap-3">
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              {loading
                ...
                : <p className="text-green-600 text-xl font-bold">{formatRupiah(todayData.income)}</p>
              }
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              {loading
                ? <div className="h-7 w-32 bg-gray-100 rounded animate-pulse mt-1" />
                : <p className="text-red-500 text-xl font-bold">{formatRupiah(todayData.expense)}</p>
              }
            </div>
          </div>

          <div className={`rounded-2xl p-4 shadow-sm border flex items-center gap-4 ${
            labaHariIni >= 0 ? 'bg-green-600 border-green-500' : 'bg-red-500 border-red-400'
          }`}>
            ...
            <div className="flex-1">
              ...
              {loading
                ...
                : <p className="text-white text-xl font-bold">{formatRupiah(labaHariIni)}</p>
              }
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-4">
            <h2 className="font-semibold text-gray-800">Tren 7 Hari Terakhir</h2>
            <button onClick={fetchData} className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
              <RefreshCw size={16} />
            </button>
          </div>
          {loading ? (
            <div className="h-48 flex items-center justify-center text-gray-400 text-sm">Memuat data...</div>
          ) : (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={weeklyData} barGap={2}>
                <XAxis dataKey="hari" tick={{ fontSize: 11 }} axisLine={false} tickLine={false} />
                <YAxis hide />
                <Tooltip formatter={(value) => formatRupiah(value)} contentStyle={{ borderRadius: 8, border: '1px solid #e5e7eb', fontSize: 12 }} />
